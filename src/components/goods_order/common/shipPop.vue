    ...
    <el-dialog :title="title" :visible.sync="UnitePop" width="550px">
      <el-form :model="dgformUnite">
        ...
        <el-form-item label="物流单号">
          <el-input
            v-model="dgformUnite.trackingNumber"
            autocomplete="off"
            style="width:400px"
            maxlength="32"
          ></el-input>
    ...
    <el-dialog title="发货" :visible.sync="MultiplePop" width="550px">
      <el-form :model="dgformMultiple">
        <el-form-item label="选择商品">
          <el-select
            v-model="dgformMultiple.goodsId"
            multiple
            placeholder="选择商品"
            style="width:300px;"
          >
        ...
        <div class="common" v-for="(item,index) in commonArr" :key="index">
          <span
            class="name"
            v-for="(ite,inx) in item.selectedList"
            :key="inx"
          >商品名称:{{ite.goodsName}}</span>
          <el-form-item label="选择物流">
...
export default {
  ...
  methods: {
    ...
    openUnitePop(orderId, item) {
      ...
      this.uniteParam.orderLogisticsList[0].orderDetailIds =
        item.goodsDetailIds;
