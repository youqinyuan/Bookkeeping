  ...
  <div class="selfSupportOrder">
    ...
    <div class="order-table">
      ...
      >
        <el-table-column
          ...
          align="center"
        ></el-table-column>
        <el-table-column prop="orderNumber" label="商品名称" align="center">
          <template slot-scope="scope">
            <div>
              {{
                scope.row.orderGoodsDetail
                  ? scope.row.orderGoodsDetail[0].goodsName
                  : ""
              }}
            </div>
            <div>
              {{
                scope.row.orderGoodsDetail
                  ? scope.row.orderGoodsDetail[0].goodsId
                  : ""
              }}
            </div>
          </template>
        ...
        >
          <template slot-scope="scope">
            <div>
              {{
                scope.row.orderGoodsDetail
                  ? scope.row.orderGoodsDetail[0].orgPrice
                  : ""
              }}
            </div>
          </template>
        ...
        <el-table-column prop label="购买用户" align="center">
          <template slot-scope="scope">
            <div>{{ scope.row.userName }}</div>
            <div>{{ scope.row.mobileNumber }}</div>
          </template>
        </el-table-column>
        <el-table-column prop label="所属商家名" align="center">
          <template slot-scope="scope">
        ...
        <el-table-column label="下单时间" width="160" align="center">
          <template slot-scope="scope">
            <span
              v-for="(item, index) in scope.row.orderTimeDetail"
              :key="index"
            >
              <span v-if="item.status == 1">{{
                item.statusTime | dateFormat
              }}</span>
            </span>
          </template>
        </el-table-column>
        ...
        <el-table-column label="操作" width="240" align="center">
          <template slot-scope="scope">
            <el-button type="text" @click="toOrderDetail(scope.row.id)"
              >查看</el-button
            >
            <el-button
              type="text"
    ...
    >
      ...
      <span slot="footer" class="dialog-footer">
        ...
        <el-button type="primary" @click="pushVerificationCode"
          >确 定</el-button
        >
      </span>
    </el-dialog>
...
export default {
  ...
  methods: {
    ...
    exportExcel() {
      ...
      getPackageOrderByExcel(parms).then((res) => {
        this.download(res);
      });
    },
    // 下载文件
    download(data) {
      ...
      a.href = url;
      a.setAttribute("download", "套餐核销订单列表.xls");
