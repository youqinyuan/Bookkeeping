<template>
  <div class="newbornZone">
    <div style="font-weight:bold;margin-bottom:10px;font-size:20px;padding-bottom:10px;">新人专区</div>
    <div class="topBox">
      <span>活动状态：</span>
      ...
      ></el-date-picker>
      <el-button type="primary" style="margin-left:40px" @click="queryNewPeople(1)">搜索</el-button>
...
import { queryNewPeople, NewPeopleUpdateStatus } from "@/network/api";
export default {
  data() {
    return {
      totalSize: 0,
  ...
  methods: {
    ...
    handleActive(status, id) {
      ...
      this.$confirm(tipsContent, tips, {
        ...
        .then(() => {
          ...
          NewPeopleUpdateStatus(this.qs.stringify(parms)).then(res => {
            if (res.data.messageCode == "MSG_1001") {
              this.queryNewPeople(1);
    ...
    createQRCode(id) {
       ...
       * id:活动id
       */
      this.$refs.createQRCode.open(1, id);
    },
    // 翻页
    pageChange(val) {
      this.currentPage = val;
