<template>
  <div>
    <el-dialog :title="title==1?'添加轮播图':'修改轮播图'" :visible.sync="DialogVisible" width="500px" center>
      ...
      >
        ...
        <div
          ...
          class="seachReasult"
          :title="item.name"
          @click="handleReasult(item)"
        >{{item.name}}</div>

        <div v-show="showNone" class="none">没有这个商品</div>
...
export default {
  ...
  methods: {
    ...
    search() {
      ...
      searchGoodsByName({
        params: { categoryId: this.categoryId, name: this.name }
      }).then(res => {
        if (res.data.messageCode == "MSG_1001") {
