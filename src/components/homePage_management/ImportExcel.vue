...
<script>
import { getHeroListExcel, importHeroListExcel } from "@/network/api";
export default {
  props: {},
  data() {
    return {
  ...
  methods: {
    ...
    download(data) {
      ...
      a.href = url;
      a.setAttribute("download", "导入虚拟用户模板.xls");
    ...
    handleSuccess(e) {
      ...
      importHeroListExcel(formData).then(res => {
        if (res.data.messageCode == "MSG_1001") {
          this.dialogVisible = false;
          this.$message.success(res.data.message);
          this.$parent.queryHeroList(1);
        } else {
