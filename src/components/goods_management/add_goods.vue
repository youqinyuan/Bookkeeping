    ...
    <el-form
      ...
      <el-row style="background: #f8f8f8; margin-top: 10px; margin-bottom: 22px;">
        ...
        <el-col :span="12" class="tle-change" v-if="goodsDetailJson.type === 2">
          <el-button @click="changeSave('essentialInfo')" type="text">{{ goodsDetailJson.essentialInfo.disabled ? '修改' : '保存' }}</el-button>
        </el-col>
      </el-row>
      ...
      <el-row>
        <el-col :span="12">
          <el-form-item label="商品名:" prop="goodsName">
            <el-input v-model="form.goodsName" maxlength="50" :disabled="goodsDetailJson.essentialInfo.disabled"></el-input>
          </el-form-item>

          <el-form-item label="分享描述:">
            <el-input v-model="form.goodsDcb" maxlength="35" :disabled="goodsDetailJson.essentialInfo.disabled"></el-input>
          ...
          <el-form-item label="商品图:" class="goods-img-fill">
            <div class="img-box">
              ...
              <div
                ...
                :key="item.key">
                <div class="del-img" @click="deleteGoodsImg('goodsImgList', index)" v-if="!goodsDetailJson.essentialInfo.disabled">
              ...
              <div
                ...
                <label class="addImgCls">
                  ...
                  <input type="file" accept="image/png,image/jpeg" ref="adImgRef" @change="handleSuccess($event, 'adImgRef', 750, 750)">
                </label>
              </div>
            </div>
      ...
      <el-row>
        ...
        <el-col :span="6">
          <el-form-item label="已抢件数:" prop="robbed">
            <el-input v-model.number="form.robbed" maxlength="6" :disabled="goodsDetailJson.essentialInfo.disabled"></el-input>
          </el-form-item>
        </el-col>
        <el-col :span="6">
      ...
      <el-row style="background: #f8f8f8; margin-top: 10px; margin-bottom: 22px;">
        ...
        <el-col :span="12" class="tle-change" v-if="goodsDetailJson.type === 2">
          <el-button @click="changeSave('stock')" type="text">{{ goodsDetailJson.stock.disabled ? '修改' : '保存' }}</el-button>
        </el-col>
      </el-row>
          ...
          <el-col class="cmmt-spec-detail">
            ...
            >
              <el-row class="spec-name">
                ...
                <el-col :span="12">
                  <el-form-item>
                    ...
                    <el-button type="text" @click="delSpecPro(index)" v-show="goodsSpec.length > 1 && index > 0">删除</el-button>
                  </el-form-item>
                </el-col>
              </el-row>
              ...
              <el-row>
                  ...
                  <el-row style="padding-left: 10px;" v-show="index === 0 && addSpecImg">
                    <el-col class="hover-col" style="width: 100%; padding-bottom: 22px;">
                      ...
                      <div
                        ...
                        class="addImgCls">
                        <i class="el-icon-plus"></i>
                        <input type="file" accept="image/png,image/jpeg" :ref="'specFileRef' + dex" @change="handleSuccess($event, 'specFileRef' + dex, 200, 200)">
                      </div>
                    </el-col>
                  </el-row>
                </el-col>
                <el-col>
                  <el-form-item>
                    <el-button v-show="!(item.specNum.length > 19)" type="text" @click="addSpecNum(index)">添加</el-button>
                    <el-button v-show="item.specNum.length > 1" type="text" @click="delSpecNum(index)">删除</el-button>
                  </el-form-item>
                </el-col>
              </el-row>
            </div>
          ...
          <el-row class="modify" v-show="!goodsDetailJson.stock.disabled">
            ...
            <el-col>
              <el-form-item>
                <el-button
                  type="text"
                  @click="batchOperation('batchPrice')"
                >价格</el-button>
              </el-form-item>
            </el-col>
      ...
      <el-dialog
        ...
        <el-form-item label="选择返现分期:">
          ...
          <el-button type="primary" @click="addCashback()" v-show="!goodsDetailJson.stock.disabled">添加</el-button>
        </el-form-item>
        <el-form-item :label="item.tle + ':'" v-for="(item, index) in cashbackDetail.cashbackList" :key="index">
          ...
          <i class="el-icon-close" @click="cashbackDelBtn(index)" v-show="!goodsDetailJson.stock.disabled"></i>
        </el-form-item>
        <div slot="footer">
          <el-button type="primary" @click="confirmCashback()" v-show="!goodsDetailJson.stock.disabled">确 定</el-button>
        </div>
      </el-dialog>
      <!-- ↑↑↑↑↑↑↑↑↑↑↑↑↑↑ 价格库存 ↑↑↑↑↑↑↑↑↑↑↑↑↑↑ -->
      ...
      <el-row style="background: #f8f8f8; margin-top: 10px; margin-bottom: 22px;">
        ...
        <el-col :span="12" class="tle-change" v-if="goodsDetailJson.type === 2">
          <el-button @click="changeSave('parityInfo')" type="text">{{ goodsDetailJson.parityInfo.disabled ? '修改' : '保存' }}</el-button>
        </el-col>
      </el-row>
      ...
      <el-row>
        <el-col :span="6">
          <el-form-item label="京东价:" prop="jinDong">
            <el-input v-model="form.jinDong" :disabled="goodsDetailJson.parityInfo.disabled" maxlength="11"></el-input>
          </el-form-item>
        </el-col>
        <el-col :span="6">
          <el-form-item label="天猫价:" prop="tianMao">
            <el-input v-model="form.tianMao" :disabled="goodsDetailJson.parityInfo.disabled" maxlength="11"></el-input>
          </el-form-item>
        </el-col>
        <el-col :span="6">
          <el-form-item label="严选考拉价:" prop="yxkl">
            <el-input v-model="form.yxkl" :disabled="goodsDetailJson.parityInfo.disabled" maxlength="11"></el-input>
          </el-form-item>
        </el-col>
      </el-row>
      <!-- ↑↑↑↑↑↑↑↑↑↑↑↑↑↑ 比价信息 ↑↑↑↑↑↑↑↑↑↑↑↑↑↑ -->
      ...
      <el-row class="banner" style="background: #f8f8f8; margin-top: 10px; margin-bottom: 22px;">
        ...
        <el-col :span="12" class="tle-change" v-if="goodsDetailJson.type === 2">
          <el-button @click="changeSave('bannerImg')" type="text">{{ goodsDetailJson.bannerImg.disabled ? '修改' : '保存' }}</el-button>
        </el-col>
      </el-row>
      <el-row>
        <el-col>
          <div class="img-box">
              ...
              <div class="img-list"
                ...
                :key="item.key"
              >
                <div class="del-img" @click="deleteGoodsImg('bannerImgList', index)" v-if="!goodsDetailJson.bannerImg.disabled">
                  <img src="@/assets/icon_del.png" alt="">
                </div>
                <img :src="item.url" alt="">
              </div>
              ...
              <div
                ...
                <i class="el-icon-plus"></i>
                <input type="file" accept="image/png,image/jpeg" ref="bannerImgFileRef" @change="handleSuccess($event, 'bannerImgFileRef', 750, 320)">
              </div>
            </div>
        </el-col>
      </el-row>
              ...
              <div style="display: flex; justify-content: flex-end;">
                <el-form-item
                  ...
                  <div class="addImgClsBtn" @click="nowAddImgAddr('detailImg')">
                    ...
                    <input type="file" accept="image/png,image/jpeg" ref="detailInfoFileRef" @change="handleSuccess($event, 'detailInfoFileRef', 750, 1000)">
                  </div>
                </el-form-item>
                <el-button type="text" v-if="goodsDetailJson.type === 2" @click="changeSave('detailInfo')">{{ goodsDetailJson.detailInfo.disabled ? '修改' : '保存' }}</el-button>
              </div>
            </el-col>
          </el-row>

          <div class="content-detail" style="position: relative;" v-for="(item, index) in bannerDetail" :key="index">
            <p style="color: #333333; font-size: 14px; line-height: 22px; word-wrap: break-word; word-break: normal;" v-if="item.resourceType === 1">{{ item.content }}</p>
            <img v-else style="display: block; width: 100%; height: auto;" :src="item.content" alt="">
            <i class="el-icon-close banner-detail-close" @click="delBannerDetail(index)" v-show="!goodsDetailJson.detailInfo.disabled"></i>
          </div>
...
export default {
  ...
  methods: {
    ...
    getBase64Image (image) {
      ...
      let type = image.src.substring(image.src.lastIndexOf('.') + 1, image.src.lastIndexOf('.') + 4)
    ...
    async completeImg (url) {
      ...
      if (this.addImgType.type === 'goodsImgList' || this.addImgType.type === 'bannerImgList') {
    ...
    addSpecNum (index) {
      ...
      if (index === 0) this.specImg.length = this.goodsSpec[index].specNum.length
    },
    ...
    setCashback (dex) {
      ...
      if (!(this.cashback[dex].cashbackList.length > 0) && this.goodsDetailJson.stock.disabled) return false
    ...
    competitorParam () {
      let arr = []
      let jD = this.form.jinDong === '' ? null : this.form.jinDong
      let tM = this.form.tianMao === '' ? null : this.form.tianMao
      let yk = this.form.yxkl === '' ? null : this.form.yxkl
    ...
    // 整合上传参数
    upParam (type) {
      let merchantId = this.form.businessOptions[0] === null ? '' : this.form.businessOptions[0] // 商家id
      let name = this.form.goodsName // 商品名
      let sharedDesc = this.form.goodsDcb // 分享描述
