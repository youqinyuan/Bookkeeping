            ...
            <div class="con">
                <el-form :model="ruleForm" :rules="rules" ref="ruleForm" label-width="140px">
                    ...
                    <el-form-item label="通用成本调控比例" prop="COST_CONTROL_RATIO">
                        <el-input style="width:250px" :disabled="disabled" v-model="ruleForm.COST_CONTROL_RATIO"></el-input>
                        <span>%</span>
                    </el-form-item>
                    <el-form-item label="用户利息比例" prop="USER_INTEREST_RATIO">
                        <el-input style="width:250px" :disabled="disabled" v-model="ruleForm.USER_INTEREST_RATIO"></el-input>
                        <span>%</span>
                    </el-form-item>
                </el-form>
            </div>
...
<script>
import {findFastInstallmentConfig,updateFastInstallmentConfig} from '@/network/api'
export default {
    props: {
        ...
        return {
            ...
            rules:{
                PLAT_RETURN_RATIO: [
                    ...
                    { pattern:/^([1-9]|[1-9]\d|1\d{1}|100)$/, message: '仅限输入1-100', trigger: 'change' }
                ],
                COST_CONTROL_RATIO: [
                    { required: true, message: '不能为空', trigger: 'change' },
                    { pattern:/^([1-9]?\d|100)$/, message: '仅限输入0-100', trigger: 'change' }
                ],
                USER_INTEREST_RATIO: [
                    { required: true, message: '不能为空', trigger: 'change' },
                    { pattern:/^(((\d|[1-9]\d)(\.\d{1,2})?)|100|100.0|100.00)$/, message:'仅输入0-100，仅限输入两位小数', trigger: 'change' }
                ]
            }
        };
    },
