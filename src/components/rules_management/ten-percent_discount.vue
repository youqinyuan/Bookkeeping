<template>
  <div class="ten-percent_discount">
    <h3>一折购最长期限购买限制</h3>
    ...
    <div v-for="(item,index) in rules" :key="index">
      <div class="itemBox">
        <div style="margin-right:20px;">
          <span>一折购最长期限购买限制次数</span>
          <el-input v-model="item.timesPerUser" style="width:110px;" :disabled="item.disabled"></el-input>
          <span>次,商品原价需小于等于</span>
          <el-input v-model="item.goodsPrice" style="width:110px;" :disabled="item.disabled"></el-input>
          <span>元</span>
        </div>
        <div class="handle">
...
export default {
  ...
  methods: {
      ...
      findListOrderAndForum({ params: parms }).then(res => {
        if (res.data.messageCode == "MSG_1001") {
          ...
          content.forEach(val => {
            ...
            json.disabled = true;
            json.id = val.id;
            json.timesPerUser = val.timesPerUser;
            json.goodsPrice = val.goodsPrice;
            json.type = val.type;
    ...
    preserve(data) {
      let rexp = /^[+]{0,1}(\d+)$/;
      if (
        !rexp.test(data.timesPerUser) ||
        !rexp.test(data.goodsPrice) ||
        data.timesPerUser > 100 ||
        data.goodsPrice > 10000
      ) {
      ...
      let parms = {
        ...
        timesPerUser: data.timesPerUser,
